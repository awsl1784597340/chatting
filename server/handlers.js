...
function makeHandleEvent(client, clientManager, chatroomManager, one2OneManager) {
  ...
  function selectMode(chatroom) {
    if (chatroomManager.getChatroomByName(chatroom))
      return chatroomManager.getChatroomByName(chatroom)
    return one2OneManager.getSingleByName(chatroom)
  }

  function ensureValidChatroom(chatroomName) {
    return ensureExists(
      () => selectMode(chatroomName),
...
module.exports = function (client, clientManager, chatroomManager, one2OneManager) {
  mode = clientMod.getClient(client)
  const handleEvent = makeHandleEvent(client, clientManager, chatroomManager, one2OneManager)
  ...
  function handleJoin(chatroomName, callback) {
    const createEntry = () => ({ event: ` 进入了 ${chatroomName}` })
  ...
  function handleOne2One(_, callback) {
    return callback(null, one2OneManager.serializeSingle())
  }
